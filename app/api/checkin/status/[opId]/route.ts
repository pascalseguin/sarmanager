...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ opId: string }> }) {
  ...
  const op = db.prepare("SELECT id, name, status, deploy_timestamp, ipp_direct_disabled FROM operations WHERE id = ?").get(opId) as any;
  ...
  return NextResponse.json({
    ...
    ippDirectDisabled: !!op.ipp_direct_disabled,
  });
}
